  ...
  require(["jquery", "cookie", "header", "footer"], () => {
  	    ...
  	    $(".list-yijinav>li>a").on("click",function(){
  	    	if($(this).attr("log") === "off"){
	  	    	...
	  	    	$(this).children().css({"background-position":"-40px -319px"})
	  	    	$(this).attr("log","on")
  	    	}else if($(this).attr("log") === "on"){
	  	    	$(this).next().animate({height:0});
	  	    	$(this).children().css({"background-position":"-10px -319px"})
	  	    	$(this).attr("log","off")
  	    	}
  	     ...
  	     $(".list-erjinav li").on("click",function(){
  	     	if($(this).attr("log") === "off"){
  	     	  //清除其他所有的on
  	     	 $(".icon-list-choice").css({"background-position":"-63px -630px"}).parent().attr("log","off");
  	     	 $(this).children().css({"background-position":"-10px -630px"});
  	     	
  	     	 $(this).attr("log","on")
  	     	}else if($(this).attr("log") === "on"){
  	     		 $(this).children().css({"background-position":"-63px -630px"});
  	     	     $(this).attr("log","off");
  	     	}
  	     })
